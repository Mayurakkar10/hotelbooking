...
public class RoomsRepository {
	...
	public RoomsModel getRoomById(int roomId) {
		try {
			return template.queryForObject("SELECT * FROM Rooms WHERE room_id = ?", new Object[] { roomId },
					roomMapper);
		} catch (Exception e) {
			return null;
		}
	...
	public boolean updateRoom(RoomsModel room) {
		...
		int rows = template.update(sql, room.getHotel_id(), room.getType_id(), room.getPrice(),
				room.getAvailability_status(), room.getImage_url(), room.getBed_count(), // New
	...
	private boolean isValidHotel(int hotelId) {
		Integer count = template.queryForObject("SELECT COUNT(*) FROM Hotels WHERE hotel_id = ?",
				new Object[] { hotelId }, Integer.class);
		return count != null && count > 0;
	}

	private boolean isValidRoomType(int typeId) {
		Integer count = template.queryForObject("SELECT COUNT(*) FROM Room_Types WHERE type_id = ?",
				new Object[] { typeId }, Integer.class);
		return count != null && count > 0;
	}
	...
	public void setRoomsAvailableAfterCheckout() {
		String sql = """
				UPDATE Rooms
